...
# need a personal access token to avoid anon rate limits, it doesn't need any scopes
...
def boshio_release_to_source(resource):
	...
	"""
	bosh_api_url = "https://bosh.io/api/v1/releases/github.com/{0}".format(resource['source']['repository'])

	with urllib.request.urlopen(urllib.request.Request(bosh_api_url, headers={"User-Agent": "curl"})) as api:
...
def get_commit(owner, repo, path):
	...
	Returns:
		tuple(str, str): The url to the submodule repo, and the sha of the commmit
	"""
	github_api_url = "https://api.github.com/repos/{0}/{1}/contents/{2}".format(owner, repo, path)
	with urllib.request.urlopen(urllib.request.Request(github_api_url, headers={"Authorization": "Basic "+AUTH})) as api:
...
def find_submodules(source):
	...
	Args:
		source(tuple(repo, branch)) - The repo and branch to find submodules for
	...
	github_api_url = "https://api.github.com/repos/{0}/{1}/git/trees/{2}?recursive=true".format(owner, repo, branch)
	try:
		with urllib.request.urlopen(urllib.request.Request(github_api_url, headers={"Authorization": "Basic "+AUTH})) as api:
			tree = json.loads(api.read().decode('utf-8'))

			if tree['truncated']:
				raise SystemExit("Unable to scan {0} for submodules.  Tree is too large".format(source[0]))
...
def get_lang(source):
	...
	Args:
		source(tuple(repo, branch)) - The repo and branch to find the languages in use.
	...
	github_api_url = "https://api.github.com/repos/{0}/{1}/languages".format(owner, repo)
...
if __name__ == "__main__":
	...
	# for p in `fly -t fr pipelines | awk '{print $1}'`; do fly -t fr gp -p ${p} > ${p}.yml; done
	...
	# Many CF "repos" are actually just collections of other repos as submodules pinned to specific commits as a "release"
	submodules = []
	with Pool(processes=10) as p:
		for s in p.map(find_submodules, sources):
			submodules += s

	all_repos = list(set([tuple(x) for x in sources] + [tuple(x) for x in submodules]))
	...
	for source in final:
		print ("{0}\t{1}\t{2}".format(source[0], source[1], ", ".join(source[2])))
