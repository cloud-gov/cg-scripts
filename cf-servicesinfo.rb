...
def get_non_sandbox_spaces()
  ...
  Helpers.cfmunge('/v2/organizations?results-per-page=100').each do |org|
    next if org['entity']['name'].include? "sandbox"

    page = 1
    more_spaces = true
    while more_spaces
      spaces = Helpers.cfmunge("/v2/organizations/#{org['metadata']['guid']}/spaces?page=#{page}&results-per-page=100")
      if spaces.length > 0
...
CSV.open("cf-orgs-services-managers-#{stamp}.csv", "wb") do |csv|
  ...
  Helpers.cfmunge('/v2/organizations?results-per-page=100&order-by=name').each do |org|
    next if org['entity']['name'].include? "sandbox"

    org_name = org['entity']['name']
    org_managers = get_org_managers(org['metadata']['guid'])

    Helpers.cfmunge("/v2/organizations/#{org['metadata']['guid']}/spaces").each do |space|
      next if exempt_space(space['entity']['name'])
