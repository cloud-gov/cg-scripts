...
for org in $orgs; do
  ...
  org_users=$(cf curl "/v3/organizations/$org_guid/users" | jq -r '[.resources[].username]')
  
  for space_guid in $(cf curl "/v3/spaces?organization_guids=$org_guid" | jq -r '.resources[].guid'); do
    space_users=$(cf curl "/v3/spaces/$space_guid/users" | jq -r '[.resources[].username]')
